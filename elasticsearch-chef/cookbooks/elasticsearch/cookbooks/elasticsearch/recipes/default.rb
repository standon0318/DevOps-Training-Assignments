...
bash 'download_and _install_elasticsearch' do
  ...
  sudo wget #{node[default['elasticsearch']['download']['checksum']]}
...
bash 'download_and _install_elasticsearch' do
  code <<-EOH
    ...
    sudo bin/elasticsearch-plugin install file:///discovery-ec2-#{node['elasticsearch']['version']}.zip
  EOH
  not_if { ::File.exist?(node['elasticsearch']['home']) }
end
...
#save iptables rules 
bash 'open_ports_for_cassandra_service' do
  code <<-EOH
